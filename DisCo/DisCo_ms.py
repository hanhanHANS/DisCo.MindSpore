...
class DisCo(nn.Cell):
    ...
    def __init__(self, base_encoder, teacher_encoder, dim=128, K=65536, m=0.999, T=0.07, mlp=False, args=None):
        ...
        if mlp:
            ...
            self.encoder_q.end_point = nn.SequentialCell([nn.Dense(dim_mlp, 2048), nn.ReLU(), nn.Dense(2048, dim)])
            self.encoder_k.end_point = nn.SequentialCell([nn.Dense(dim_mlp, 2048), nn.ReLU(), nn.Dense(2048, dim)])
